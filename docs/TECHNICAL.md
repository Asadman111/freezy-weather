...
3) Try to assemble a release `./gradlew assembleBasicRelease -Pbreezy`. /!\ Only the Freezy Weather organization can distribute releases with the `breezy` parameter. Forks must not include this parameter when building, and use their own branding.
...
1) Save crash log in a text file `stacktrace.txt` and remove things like `2023-08-14 21:28:58.229 12804-12804 View org.freezyweather.debug` if they are present.
