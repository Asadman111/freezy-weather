...
**New features**
- Content provider: allows (with your permission) other apps to query your weather data. [Read the announcement](https://github.com/freezy-weather/freezy-weather/discussions/2089)
- New broadcast: you can use `org.freezyweather.ACTION_UPDATE_NOTIFIER` (or `org.freezyweather.debug.ACTION_UPDATE_NOTIFIER` with the debug build) to be notified of updated locations (most common use case is coupled with the content provider)
...
- Widgets - Minor fixes
- Wallpaper - Due to some people running outdated versions of Freezy Weather just to see some gimmicks on their wallpaper, we bring back wallpaper animations behind a dangerous disabled-by-default option. We STRONGLY advise against enabling them.
...
**Removed features**
- Sun & Moon data from sources. Will now always be computed by Freezy Weather for consistency
