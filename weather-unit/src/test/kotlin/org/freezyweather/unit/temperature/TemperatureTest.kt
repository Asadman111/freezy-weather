...
package org.freezyweather.unit.temperature
...
class TemperatureTest {
    ...
    fun `format Celsius with narrow unit width with Android translations`() = runTest {
        ...
        val context = mockk<Context>().apply {
            ...
            every {
                getString(
                    org.freezyweather.unit.R.string.temperature_c_nominative_narrow,
                    capture(formattedNumberSlot)
                )
            } answers { "${formattedNumberSlot.captured}°" }
            every {
                getString(
                    org.freezyweather.unit.R.string.temperature_k_nominative_narrow,
                    capture(formattedNumberSlot)
                )
    ...
    fun `format Celsius with short unit width with Android translations`() = runTest {
        ...
        val context = mockk<Context>().apply {
            ...
            every {
                getString(
                    org.freezyweather.unit.R.string.temperature_c_nominative_short,
                    capture(formattedNumberSlot)
                )
            } answers { "${formattedNumberSlot.captured} °C" }
            every {
                getString(
                    org.freezyweather.unit.R.string.temperature_k_nominative_short,
                    capture(formattedNumberSlot)
                )
            } answers { "${formattedNumberSlot.captured} K" }
        }
    ...
    fun `format Celsius with long unit width with Android translations`() = runTest {
        ...
        val context = mockk<Context>().apply {
            ...
            every {
                getString(
                    org.freezyweather.unit.R.string.temperature_c_nominative_long,
                    capture(formattedNumberSlot)
                )
            } answers { "${formattedNumberSlot.captured} degrés Celsius" }
            every {
                getString(
                    org.freezyweather.unit.R.string.temperature_k_nominative_long,
                    capture(formattedNumberSlot)
                )
    ...
    fun `get temperature display name`() = runTest {
        val context = mockk<Context>().apply {
            every { getString(any()) } returns "FAILED"
            every {
                getString(org.freezyweather.unit.R.string.temperature_f_display_name_short)
            } answers { "°F" }
            every {
                getString(org.freezyweather.unit.R.string.temperature_f_display_name_long)
