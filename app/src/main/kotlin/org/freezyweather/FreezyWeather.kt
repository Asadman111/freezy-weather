...
import org.freezyweather.common.utils.AndroidSignatureFinder
...
@HiltAndroidApp
class FreezyWeather : Application(), Configuration.Provider {

    companion object {

        lateinit var instance: FreezyWeather
            private set
    ...
    private val activitySet: MutableSet<FreezyActivity> by lazy {
        HashSet()
    }
    var topActivity: FreezyActivity? = null
        private set
    ...
    fun addActivity(a: FreezyActivity) {
        activitySet.add(a)
    }

    fun removeActivity(a: FreezyActivity) {
        activitySet.remove(a)
    }

    fun setTopActivity(a: FreezyActivity) {
        topActivity = a
    }

    fun checkToCleanTopActivity(a: FreezyActivity) {
        if (topActivity === a) {
            topActivity = null
        }
    }
     ...
     * to you.
     * You're allowed to make a fork, but you're NOT allowed to impersonate the "Freezy Weather" app.
     * Use your own app name. See instructions in the README file, License section.
     */
    val isSignedByFreezy: Boolean
        get() {
